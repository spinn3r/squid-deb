...
default_all(void)
{
	...
	default_line("maximum_object_size 20480 KB");
	...
	default_line("logfile_daemon /usr/lib/squid/logfile-daemon");
	default_line("cache_log /var/log/squid/cache.log");
	default_line("cache_store_log /var/log/squid/store.log");
	...
	default_line("mime_table /usr/share/squid/mime.conf");
	...
	default_line("netdb_filename /var/spool/squid/logs/netdb.state");
	...
	default_line("diskd_program /usr/lib/squid/diskd-daemon");
#if USE_UNLINKD
	...
	default_line("icon_directory /usr/share/squid/icons");
	default_line("global_internal_static on");
	default_line("short_icon_urls off");
	default_line("error_directory /usr/share/squid/errors/en");
...
defaults_if_none(void)
{
	...
	if (check_null_cachedir(Config.cacheSwap)) {
		default_line("cache_dir ufs /var/spool/squid 100 16 256");
	}
#if USE_WCCPv2
